...
def _parse_decision(decision_raw: object) -> Decision:
    if isinstance(decision_raw, Decision):
        return decision_raw
    try:
        return Decision(str(decision_raw))
    except Exception:
...
def _parse_approval(approval_raw: object) -> ApprovalDecision:
    if isinstance(approval_raw, ApprovalDecision):
        return approval_raw
    try:
        return ApprovalDecision(str(approval_raw).lower())
    except Exception:
...
async def _maybe_doctor_summary(patient_data: dict, model: str, assessment_details: dict) -> dict | None:
...
def _build_output(*,
                  ...
                  claims_with_citations: dict | None = None) -> dict:
    ...
    if interrupt_stage:
        out["interrupt_stage"] = interrupt_stage if isinstance(interrupt_stage, str) else str(interrupt_stage)
    audit = AuditBundle(
        assessment=assessment,
        clinical_reasoning=clinical_reasoning,
        validator=_dump(validator),
        safety_validation=safety_validation,
        prescribing_considerations=presc,
        research_context=research,
        diagnosis=diagnosis,
...
def state_validator_op(patient_data: dict, regimen_text: str, safety: dict | None) -> dict:
    patient = PatientState(**patient_data)
    val = state_validator(patient, regimen_text, safety)
    if hasattr(val, "model_dump"):
        try:
            return val.model_dump()  # type: ignore[attr-defined]
        except Exception:
            pass
    return val if isinstance(val, dict) else {"passed": True, "severity": "low"}
...
def pharmacist_refinement_op(recommendation: dict, safety_result: dict) -> dict:
    ...
    try:
        ...
        alts = list((recommendation or {}).get("alternatives", []) or [])
        rec_text = (
            f"{recommendation.get('regimen')} {recommendation.get('dose')} {recommendation.get('frequency')} x {recommendation.get('duration')}"
    ...
    for alt in alts:
        if isinstance(alt, str) and alt.strip() and alt.strip() != rec_text:
            ...
            break
    return {
        "approval": str((safety_result or {}).get("approval_recommendation", "undecided")),
        "original": rec_text,
...
def _should_verify(clinical_reasoning: dict, validator: object, safety_result: dict | None, confidence_threshold: float = 0.8) -> bool:
    try:
    ...
    try:
        risk_raw = (safety_result or {}).get("risk_level") if isinstance(safety_result, dict) else None
...
async def _run_agent_stream_with_retry(agent: Agent, prompt: str) -> object:
    if RunConfig is not None:
        try:
            ...
            stream = Runner.run_streamed(agent, prompt, run_config=RunConfig(temperature=temperature))  # type: ignore
        except Exception:
...
async def clinical_reasoning(patient_data: dict, model: str = "gpt-4.1", assessment_details: dict | None = None) -> dict:
    patient = PatientState(**patient_data)
    prompt = make_clinical_reasoning_prompt(patient, assessment_details)
    agent: Agent = make_clinical_reasoning_agent(model)
    out: ClinicalReasoningOutput = await _run_agent_stream_with_retry(agent, prompt)  # type: ignore
    return {**out.model_dump(), "model": agent.model, "version": "v1", "narrative": out.as_narrative()}
...
async def safety_validation(
    ...
    if recommendation:
        try:
            ...
            rec_text = f"{recommendation.get('regimen')} {recommendation.get('dose')} {recommendation.get('frequency')} x {recommendation.get('duration')}"
    try:
        ...
        if clinical_reasoning_context and decision_enum == Decision.recommend_treatment:
            proposed = str(clinical_reasoning_context.get("proposed_regimen_text", "") or "").strip()
            if proposed:
                rec_text = proposed
    except Exception:
        pass
    prompt = make_safety_validation_prompt(patient, decision, rec_text, clinical_reasoning_context)
    agent: Agent = make_safety_validation_agent(model)
    out: SafetyValidationOutput = await _run_agent_stream_with_retry(agent, prompt)  # type: ignore
    return {**out.model_dump(), "model": agent.model, "version": "v1", "narrative": out.as_narrative()}
...
async def prescribing_considerations(patient_data: dict, region: str, model: str = "gpt-4.1") -> dict:
    patient = PatientState(**patient_data)
    considerations = [
    ...
    try:
        extra = await web_research(
            "Latest regional resistance and any UTI guideline updates (concise)", region, model,
        )
    ...
    for consideration in considerations:
        if not consideration.startswith("Patient-specific:") and not consideration.startswith("Current resistance intelligence:"):
...
async def deep_research_diagnosis(
    ...
    try:
        patient = PatientState(**patient_data)
        assessment = assess_uti_patient(patient)
        xml = make_diagnosis_xml_prompt(patient, assessment, doctor_reasoning, safety_validation_context)
        ...
        logger.error(f"Deep research diagnosis failed: {e}")
        return {
            "diagnosis": f"Research diagnosis unavailable due to error: {e!s}",
            "citations": [],
            "model": model,
            "error": str(e),
        }
    ...
    try:
        ...
        if isinstance(rec_obj, dict):
            try:
                ...
                rec_text = (
                    f"{rec_obj.get('regimen')} {rec_obj.get('dose')} {rec_obj.get('frequency')} x {rec_obj.get('duration')}"
        ...
        def _format_assessment_narrative(decision: str, rec_text: str, rationale: list[str], follow_up: str | None) -> str:
            narrative_lines = [
                f"Decision: {decision}",
        ...
        rd["narrative"] = _format_assessment_narrative(decision, rec_text, rationale, follow_up)
        return rd
    except Exception as e:
        return {
...
async def follow_up_plan(patient_data: dict) -> dict:
    try:
        ...
        def _format_follow_up_narrative(monitoring: list[str], special_instructions: list[str]) -> str:
...
async def uti_complete_patient_assessment(patient_data: dict, model: str = "gpt-4.1") -> dict:
    try:
        ...
        decision = _parse_decision(assessment_result.get("decision", Decision.no_antibiotics_not_met))
        
        if _strict_interrupts_enabled() and decision in {Decision.refer_complicated, Decision.refer_recurrence}:
            doctor_summary = await _maybe_doctor_summary(patient_data, model, assessment_details)
            return _build_output(
                path=OrchestrationPath.deterministic_interrupt,
                assessment=assessment_result,
                clinical_reasoning=doctor_summary or {"reasoning": ["Deterministic referral triggered. No agentic processing."], "confidence": 1.0},
        ...
        if decision == Decision.recommend_treatment:
            clinical_result = await clinical_reasoning(patient_data, model, assessment_details)
        else:
            if _strict_interrupts_enabled():
                ...
                return _build_output(
                    ...
                    assessment=assessment_result,
                    clinical_reasoning=doctor_summary or {"reasoning": ["No antibiotics per algorithm"], "confidence": 1.0},
                    safety_validation=None,
                    presc=None,
                    research=None,
                    diagnosis=None,
            ...
            clinical_result = {"reasoning": ["Referral/no antibiotics per algorithm"], "confidence": 1.0}
        ...
        if decision == Decision.recommend_treatment:
            ...
            try:
                sa = _parse_approval((safety_result or {}).get("approval_recommendation", "undecided"))
                if _strict_interrupts_enabled() and sa in [ApprovalDecision.reject, ApprovalDecision.do_not_start, ApprovalDecision.deny, ApprovalDecision.refer_no_antibiotics]:
                    return _build_output(
                ...
                if sa in [ApprovalDecision.modify, ApprovalDecision.conditional, ApprovalDecision.reject, ApprovalDecision.do_not_start, ApprovalDecision.refer_no_antibiotics]:
                    patient = PatientState(**patient_data)
                    refine_prompt = make_reasoning_refinement_prompt(patient, clinical_result, safety_result)
                    agent = make_clinical_reasoning_agent(model)
                    refined_out: ClinicalReasoningOutput = await _run_agent_stream_with_retry(agent, refine_prompt)  # type: ignore
                    if refined_out is not None and hasattr(refined_out, "model_dump"):
                        try:
                            ...
                            refined_dict = dict(refined_out)
                        clinical_result = {**refined_dict, "model": agent.model, "version": "v1"}
            except Exception:
                pass

        safety_approval = _parse_approval((safety_result or {}).get("approval_recommendation", "undecided"))
        rec = assessment_result.get("recommendation") or {}
        try:
            rec_text = Recommendation(**rec).as_text() if isinstance(rec, dict) else "None"
        except Exception:
            rec_text = (
                f"{rec.get('regimen')} {rec.get('dose')} {rec.get('frequency')} x {rec.get('duration')}"
                if rec else "None"
            )
        ...
        if decision == Decision.recommend_treatment and rec:
            if safety_approval == ApprovalDecision.approve:
                proposed = str(clinical_result.get("proposed_regimen_text", "") or "").strip()
                ...
                try:
                    ...
                    if chosen_alt:
                        ...
                        consensus_recommendation = f"Modify regimen: {chosen_alt} (per safety validation)"
                    else:
                        finalized_regimen_text = rec_text
                        consensus_recommendation = f"Modify regimen: {rec_text} (see safety validation)"
                except Exception:
                    finalized_regimen_text = rec_text
                    consensus_recommendation = f"Modify regimen: {rec_text} (see safety validation)"
            elif safety_approval in [ApprovalDecision.reject, ApprovalDecision.do_not_start, ApprovalDecision.deny]:
        ...
        if decision == Decision.recommend_treatment:
            if isinstance(safety_result, dict):
                approval = _parse_approval((safety_result or {}).get("approval_recommendation", "undecided"))
                if approval in [ApprovalDecision.reject, ApprovalDecision.do_not_start, ApprovalDecision.deny, ApprovalDecision.refer_no_antibiotics]:
        ...
        try:
            val_passed = bool(validator.get("passed", True)) if isinstance(validator, dict) else bool(getattr(validator, "passed", True))
        except Exception:
            val_passed = True
        try:
            val_severity = (
                str(validator.get("severity", "")).lower() if isinstance(validator, dict)
        ...
        if _strict_interrupts_enabled() and val_severity == "high":
            return _build_output(
                ...
                validator=validator if isinstance(validator, dict) else getattr(validator, "model_dump", lambda: validator)(),
                model=model,
                patient_inputs=patient_data,
                human_escalation=True,
        ...
        if val_passed:
            ...
            summary_task = web_research("Latest UTI guideline updates and resistance (concise)", region, model)
            diagnosis_task = deep_research_diagnosis(
                patient_data,
                model,
                doctor_reasoning=clinical_result,
                safety_validation_context=safety_result if isinstance(safety_result, dict) else None,
            )
            presc_result, summary_result, diagnosis_result = await asyncio.gather(
                presc_task, summary_task, diagnosis_task,
            )
        ...
        should_verify = _should_verify(clinical_result, validator, safety_result, confidence_threshold=0.8)

        if should_verify:
            try:
                verifier_agent = make_verifier_agent(model)
                verifier_prompt = make_verifier_prompt(final_snapshot)
                verification_report = await _run_agent_stream_with_retry(verifier_agent, verifier_prompt)
                if verification_report is not None and hasattr(verification_report, "model_dump"):
                    try:
        ...
        try:
            ...
            if claims_output is not None and hasattr(claims_output, "model_dump"):
                try:
                    ...
                    claims_output = dict(claims_output)
        except Exception:
            claims_output = None
        ...
        logger.error(f"Final consolidated agent failed: {e}")
        return {"error": f"Final consolidation failed: {e!s}", "model": model}
